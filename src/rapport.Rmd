...
Dans le cadre de notre 3ème année de licence informatique, il a été demandé aux membres de réaliser un projet d'analyse de jeu données dans le cadre de notre UE traîtement numérique des données.
L'objet dans ce rapport est un ensemble de voitures représenter par le fichier fourni "275-cars.txt" qui contrairement à son nom, contient les données de 261 voitures.
...
```{r fig-9-inertia-gain-ward}
plot(cars.hc.ward.inertia[1:20], type = "s", 
     xlab = "Nombre de classes", 
     ylab = "Inertie", 
     main = "Fig. 9 : Inertie du dendrogramme en fonction du nombre de classes")
```
...
```{r fig-10-hc-ward-inertia-critical-points}
plot(cars.hc.ward.inertia[1:20], type = "s",
     xlab = "Nombre de classes", 
     ylab = "Inertie", 
     main = "Fig 10. Inertie du dendrogramme avec plus grands gains entourés")
points(c(2, 4), cars.hc.ward.inertia[c(2, 4)], col = c("green", "red"), cex = 2, lwd = 3)
```

```{r fig-11-hc-ward-partitions}
par(mar=c(1,4.5,2,0))
plot(cars.hc.ward, labels = FALSE, 
     main = "Fig. 11 : Partitions en 2 ou 4 classes", 
     ylab = "Hauteur", 
     hang = -1)
...
Comme énoncé dans la section *3. Prétraitement* **[à vérifier]**, la variable `brand` peut être utilisée comme variable supplémentaire lors de cette opération, celle-ci étant la seule variable catégorielle du jeu de données.
...
On obtient alors sept composantes principales allant de `Dim1` à `Dim7`.
...
L'ensemble des variables se trouvent relativement loin de l'origine, elles sont donc bien représentées par ce plan factoriel.
...
Le dendrogramme suggère une solution à 3 groupes, on peut également les visualiser sur l'ACP
...
Pour chaque groupe, les 5 meilleurs individus les plus proches du centre du cluster sont affichés et sont appelés parangons.
La distance entre chaque individu et le centre du groupe est fournie.
Par exemple, les individus représentatifs pour le groupe 2 inclus les individus/véhicules suivants : 87, 34, 229, 208 et 159.

...
**[Jack : Question bête, mais à quoi sert "head()" ici? pour moi ça affiche rien, à vrai dire, j'arrive même pas à faire print(cars)]**
```{r read_file}
...
**[Jack : J'ai voulu créer une fonction de pretaitement, mais si vous trouvez que c'est en trop, vous pouvez enlever]**
```{r na_values}
#' Fonction permettant de retirer des lignes avec valeurs manquantes si il y'en a
#'
#' @param dataset le dataset dont les valeurs sont à éxaminer
#' @return result un dataset sans les valeurs na
remove_na = function(dataset){
  result=dataset
  if (sum(is.na(dataset))>0)
  {
    print("le dataset a des ligne avec valeurs manquante")
    print("retrait des lignes avec valeurs manquante")
    result=subset(dataset, !is.na(dataset))
  } else if (sum(is.na(dataset))==0) {
    print("le dataset n'a pas de ligne avec valeurs manquante")
  }
  return(result)
}
cars = remove_na(cars)

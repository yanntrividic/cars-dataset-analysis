...
En observant les résultat d'une table de contingence correspondant à cette hypothèse, cette dernière s'en voit invalidée ; les résultats ne soutiennent pas l'idée que les classes trouvées correspondraient parfaitement aux valeurs de `brand`.
...
Il pourrait donc être intéressant de voir si, en groupant les voitures européennes et japonaises, on obtient un meilleur partitionnement avec $k = 2$.

```{r contingency-table-brand-cluster2}
# TODO grouper les européennes et américaines, regarder ce que ça donne avec k = 2
```

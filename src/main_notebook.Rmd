...
cars.cah.ward = hclust(cars.dist, method="ward.D2")
cars.cah.ward.inertia = sort(cars.cah.ward$height, decreasing = T)

plot(cars.cah.ward.inertia[1:20], type = "s", 
...
points(c(2, 4), cah.ward.inertia[c(2, 4)], col = c("green", "red"), cex = 2, lwd = 3)
```

```{r hc-ward-partitions}
par(mar=c(1,4.5,2,0))
plot(cars.cah.ward, labels = FALSE, main = "Partitions en 2 ou 4 classes", xlab = "", ylab = "Hauteur", sub = "", hang = -1)
rect.hclust(cars.cah.ward, 2, border = "green")
rect.hclust(cars.cah.ward, 4, border = "red")
```
...
cut_affiche_table_contingence = function(cah_result, cut_vector, factor){
    for(cut in cut_vector){
        cat(paste("\nCut =", cut, "\n"))
        c = cutree(cah_result, cut)
        afficher_table_contingence_clusters(factor, c)
    }
}

# on coupe au niveau 4 et affichons les tables de contingences
cut_affiche_table_contingence(cars.cah.ward, c(2, 4), cars$brand) 
```
...
cut_affiche_table_contingence(cars.cah.ward, 4, cars.combined.brand)
```
...
cars.cah.ward.cut4.tree2 <- cut(as.dendrogram(cars.cah.ward), h = 13)$lower[[3]]
# puis on extrait les indices
cars.cah.ward.cut4.tree2.ind <- as.numeric(names(cutree(cars.cah.ward.cut4.tree2, k=1)))

dist = dist(cars.scaled[cars.cah.ward.cut4.tree2.ind, -8])
...
cut_affiche_table_contingence(cah.ward, c(2, 3, 4), cars$brand[cars.cah.ward.cut4.tree2.ind])
...
afficher_table_contingence_clusters(cars.combined.brand[cars.cah.ward.cut4.tree2.ind], cah.ward.cut3)
```
...
```{r hc-single-link}
cars.cah.min = hclust(cars.dist, method="single")
par(mar=c(1,4.5,2,0))
plot(cars.cah.min, labels = FALSE, main = "Dendrogramme suite à la CAH avec lien minimum", xlab = "", ylab = "Hauteur", sub = "", hang = -1)
```

```{r hc-complete}
cars.cah.max = hclust(cars.dist, method="complete")
par(mar=c(1,4.5,2,0))
plot(cars.cah.max, labels = FALSE, main = "Dendrogramme suite à la CAH avec lien maximum", xlab = "", ylab = "Hauteur", sub = "", hang = -1)

cars.cah.max.inertia = sort(cars.cah.max$height, decreasing = T)

plot(cars.cah.max.inertia[1:20], type = "s", 
     xlab = "Nombre de classes", 
     ylab = "Inertie", 
     main = "Inertie du dendrogramme en fonction du nombre de classes")
...
plot(cars.cah.max.inertia[1:20], type = "s",
     ...
     ylab = "Inertie", 
     main = "Inertie du dendrogramme avec plus grands gains entourés")
points(c(2, 3, 4, 5), cars.cah.max.inertia[c(2, 3, 4, 5)], col = c("green", "red", "blue", "orange"), cex = 2, lwd = 3)

# affichage
par(mar=c(1,4.5,2,0))
plot(cars.cah.max, labels = FALSE, main = "Partitions en 2, 3, 4 ou 5 classes", xlab = "", ylab = "Hauteur", sub = "", hang = -1)
rect.hclust(cars.cah.max, 2, border = "green")
rect.hclust(cars.cah.max, 3, border = "red")
rect.hclust(cars.cah.max, 4, border = "blue")
rect.hclust(cars.cah.max, 5, border = "orange")

# affichage des tables de contingence
cut_affiche_table_contingence(cars.cah.max, c(2, 3, 4, 5), cars$brand)
# TODO: interpreter
```
...
Grêce à l'ACP, nous tenons compte des liaisons entre les variables.

```{r q1-8-2 out.width="500px", fig.align='center', echo=FALSE}

library("FactoMineR")
library("factoextra")
# paramètres à ajuster
#catdes(cars[, c(1:8)], 8)
#CPA marche mieux avec des variables quantitatives
...
On obtient alors 7 composantes principales allant de PC1 à PC7.

Dans ces résultats , la première composante (PC1) présente une forte association positive avec la variable "mpg", elle est cependant moins importante avec "tme.to.60" et "year".
En ce qui concerne les variables restantes , les associaitions sont négatives .

Le tableau des valeurs propres est un outil permettant de déterminer quels axes à prendre en compte pour l'ACP.
Il est important que les valeurs propres des axes retenus restituent une "bonne proportion" de l'analyse.
Cela signifie que la somme de l'inertie expliquée par chacun des axes ("Proportion of variance") représente une partie importante de l'inertie totale.
C'est pour cette raison que nous allons retenir PC1 et PC2 qui présentent, respectivement, et 72 et 13 % d'inertie , soit un total 85%.
Elles regroupent 85% d'information du jeu d'origine.
...
cars.pca <- PCA(cars[c(1:7)], scale.unit = TRUE, ncp = 2, graph =FALSE)
...
\#\#[TODO avec CAH :Réalisez une ACP sur les résultats obtenus à l'aide de la CAH.
Coloriez les résultats de l'ACP en fonction d'une variable]\#\#

Le graphique nous montre que sur les 7 variables représentées, 6 sont corrélées avec Dim1, une observation cohérente puisque cette composante a une inertie de 72% dans l'ensemble du data set.
Parmis les variables corrélées négativement nous trouvons : time.to.60(temps nécesssaire pour aller de 0 à 60 mph) et mpg (distance parcourue avec un gallon d'essence).
Elle mesure donc principalement la capacité d'accélération du véhicule.
Parmi les variables corrélées positivement nous avons : hp, cylinders, cubicinches et weight.

Pour Dim2, l'unique variable qui lui est corrélée est year , et elle est corrélée de façon positive

L'ensemble des variables se trouvent relativement loin de l'origine, elles sont donc bien représentés par l'ACP.
Nous pouvons vérifier cela en visualisant le cos2 des variables sur toutes les dimensions en utilisant le package corrplot

```{r ACP qualité de représentation }
...
Il faut tout d'abord réaliser l'ACM du tableau de données

```{r HCPC }
cars.pca <- PCA(cars[c(1:7)],scale. = TRUE)
cars.hcpc=HCPC(cars.pca,graph=TRUE)
#cars.hcpc = HCPC(cars.pca)
#HCPC(cars, nb.clust = 0, graph = TRUE)
```

```{r visualisation de l'HCPC}
fviz_dend(cars.hcpc, 
...
\`\`\`{r HCPC classification hiérarchique et le plan des facteurs} \# Principal components + tree plot(cars.hcpc, choice = "3D.map")

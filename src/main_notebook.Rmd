...
```{r summary}
library(psych) # On appelle la librairie avant de l'utiliser
...
afficher_table_contingence_clusters(cars$brand, cars.cluster3$cluster)
...
afficher_table_contingence_clusters(cars.combined.brand, cars.cluster2$cluster)
```
...
<<<<<<< HEAD
Cette même observation peut-être faire en lisant la table de contingence
=======
...
>>>>>>> bde8d0dcb7dc212cf58ed1e2c0075193935fad37
...
Ce problème doit donc être abordé préalablement à l'exécution d'une CAH.

Utiliser les parangons résultants d'une CAH à 50 ou 100 classes, puis les labelliser avec leur numéro de ligne + Brand
...
```
<<<<<<< HEAD
Il est également possible de "couper" un dendrogramme au niveau de seuil spécifique pour avoir plus de clarté. On s'aperçoit également qu'il est possible de diviser ce graphique en quatre groupes.
```{r CAH découpage - matérialisation des groupes out.width="500px", fig.align='center', echo=FALSE}

#centrage réduction des données
#pour éviter que variables à forte variance pèsent indûment sur les résultats
cars.cr <- scale(cars[, -8],center=T,scale=T)
#matrice des distances entre individus
d.cars<- dist(cars.cr)
#CAH - critère de Ward
#method = « ward.D2 » correspond au vrai critère de Ward
#utilisant le carré de la distance
cah.ward <- hclust(d.cars,method="ward.D2")
...
```
##[dire dans quels groupes appartiennent les véhicules , j'ai un doute au niveau de l'interprétation ]
=======
Afin d'améliorer la lisiblité, il est possible de découper un dendrogramme, sur le graphique précédent, on distingue grossièrement quatre groupes pour les seuils <15, on va utiliser ces groupes pour créer notre découpage.
...
cah.ward= hclust(d.cars,method="ward.D2")
plot(cah.ward, xlab="Individus", ylab="Seuil", main="Dendogramme découpé", 
     col.axis="blue", col.main="blue") # Affichage dendrogramme
rect.hclust(cah.ward,k=4, border="blue") # Dendrogramme avec matérialisation des groupes
groupes.cah= cutree(cah.ward,k=4) # Découpage en 4 groupes
print(groupes.cah) # Liste des groupes

# Version alternative avec librairie Factoextra
library(factoextra)

fviz_dend(cah, k=4, xlab="Individus", ylab="Seuil", main="Dendogramme découpé (factoextra)", 
     col.axis="blue", col.main="blue", horiz = TRUE)

dend_plot=fviz_dend(cah)
dend_data=attr(dend_plot, "dendrogram")
dend_cuts=cut(dend_data, h = 14 )
p1=fviz_dend(dend_cuts$lower[[1]])
p2=fviz_dend(dend_cuts$lower[[2]])
p3=fviz_dend(dend_cuts$lower[[3]])
p4=fviz_dend(dend_cuts$lower[[4]])
plot(p1, main="p1")
plot(p2, main="p2")
plot(p3, main="p3")
plot(p4, main="p4")
...
Dans ces résultats , la première composante (PC1) présente une forte association positive avec la variable "mpg", elle est cependant moins importante avec "tme.to.60" et "year". En ce qui concerne les variables restantes , les associaitions sont négatives .

Le tableau des valeurs propres est un outil permettant de déterminer quels axes à prendre en compte pour l'ACP. Il est important que les valeurs propres des axes retenus restituent une “bonne proportion” de l’analyse. Cela signifie que la somme de l’inertie expliquée par chacun des axes ("Proportion of variance") représente une partie importante de l’inertie totale. C'est pour cette raison que nous allons retenir PC1 et PC2 qui présentent, respectivement, et 72 et 13 %  d'inertie , soit un total 85%. Elles regroupent 85% d'information du jeu d'origine.
...
```
##[TODO avec CAH :Réalisez une ACP sur les résultats obtenus à l’aide de la CAH. Coloriez les résultats de l’ACP en fonction d'une variable]##
...
Parmis les variables corrélées négativement nous trouvons : time.to.60(temps nécesssaire pour aller de 0 à 60 mph) et mpg (distance parcourue avec un gallon d'essence). Elle mesure donc principalement la capacité d'accélération du véhicule.
...
``` 
```{r HCPC  classification hiérarchique et le plan des facteurs}
# Principal components + tree
plot(cars.hcpc, choice = "3D.map")
``` 
##[le graph 3d on peut le supprimer , il est pas  tres lisible]

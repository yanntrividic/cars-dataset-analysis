...
corrplot(cars.correlations,addCoef.col = "grey", 
         number.cex = 0.5, 
         title="Fig 3. Corrélogramme du jeu de données", 
         mar=c(0,0,1,0))
```

```{r fig-4-silhouette}
fviz_nbclust(cars[-8], FUNcluster = kmeans, method = c("silhouette"), k.max = 15, nboot = 100) + labs(title="Fig. 4 : Nombre de classes optimal") +
...
plot(cars.hc.ward.inertia[1:20], type = "s",
     xlab = "Nombre de classes", 
     ylab = "Inertie", 
     main = "Fig 11. Inertie du dendrogramme avec plus grands gains entourés")
...
```{r fig-13-var-pca}
plot(cars.pca, choix="var", axes=1:2, title="Fig. 13 : Variables représentées sur le premier plan factoriel")
```

```{r fig-14-pca-representation-quality}
fviz_cos2(cars.pca, choice = "var", axes = 1:2, title = "Fig. 14 : Cos2 des variables du premier plan factoriel")
```
...
fviz_cluster(cars.hcpc,
             ...
             main = "Fig. 16 : Classes trouvées par l'HCPC projetées sur le premier plan factoriel"
             )
```
